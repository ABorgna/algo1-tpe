...
\begin{problema}{venderL}{l: Local, p:Pedido}{}
	\requiere[noHabiaPedidos]{|ventas(l)|==0 \Rightarrow numero(p)==1}
	...
	\asegura{\neg sePuede(pre(l),p) \Rightarrow \\
		(\forall e \selec empleados(l)) energiaEmpleado(l,e) == energiaEmpleado(pre(l))}
	\asegura{\neg sePuede(pre(l),p) \Rightarrow \\
	...
	\asegura{sePuede(pre(l),p) \Rightarrow \\
		(\forall e \selec empleados(l), e \neq atendio(p)) energiaEmpleado(l,e) == energiaEmpleado(pre(l))}
	\asegura{sePuede(pre(l),p) \Rightarrow \\
